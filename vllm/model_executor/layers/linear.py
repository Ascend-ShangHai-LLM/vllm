...
@CustomOp.register("intern_column_parallel_linear")
class InternColumnParallelLinear(LinearBase):
    ...
    def __init__(
        ...
        *,
        ...
        self.output_size_per_partition = self.shard_sizes[self.tp_rank]
        self.output_partition_sizes = [self.output_size_per_partition]
    ...
    def weight_loader(self, param: Parameter, loaded_weight: torch.Tensor):
        ...
        if output_dim is not None and not is_sharded_weight:
            shard_size = param_data.shape[output_dim]
            start_idx = sum(self.shard_sizes[:self.tp_rank])
            loaded_weight = loaded_weight.narrow(output_dim, start_idx,
                                                 shard_size)
...
class InternQKVParallelLinear(InternColumnParallelLinear):
    """Linear layers for the attention's QKV transformation.
    ...
    def __init__(
        ...
        *,
        ...
        assert self.tp_size <=  self.total_num_kv_heads
        ...
        self.shard_num_heads = [get_distribute_size(self.total_num_heads, self.tp_size, rank) for rank in range(self.tp_size)]
        self.shard_num_kv_heads = [get_distribute_size(self.total_num_heads, self.tp_size, rank) for rank in range(self.tp_size)]
        self.num_heads = self.shard_num_heads[self.tp_rank]
        self.num_kv_heads = self.shard_num_kv_heads[self.tp_rank]
        self.shard_q_sizes = [ _num_head * self.head_size for _num_head in self.shard_num_heads] 
        self.shard_qkv_sizes = [(_num_head + _num_kv_head* 2) * self.head_size for _num_head, _num_kv_head in zip(self.shard_num_heads, self.shard_num_kv_heads)]
        
        input_size = self.hidden_size
        output_size = self.shard_qkv_sizes[self.tp_rank] 

        super().__init__(input_size=input_size,
                         output_size=output_size,
                         shard_sizes=self.shard_qkv_sizes,
                         bias=bias,
                         gather_output=False,
                         skip_bias_add=skip_bias_add,
                         params_dtype=params_dtype,
    ...
    def weight_loader(self,
        ...
        if output_dim is not None:
            if loaded_shard_id == "q":
                ...
                shard_offset = (self.num_kv_heads +
                                self.num_kv_heads) * self.head_size
                shard_size = self.num_kv_heads * self.head_size
            ...
            param_data = param_data.narrow(output_dim, shard_offset,
                                           shard_size)
            shard_id = self.tp_rank
            shard_size = self.shard_q_sizes[shard_id]
            start_idx = sum(self.shard_q_sizes[:shard_id])
            
            #print(f"=====================rank {self.tp_rank} loaded_weight.shape {loaded_weight.shape} output_dim {output_dim} start_idx {start_idx} shard_size {shard_size}")
            loaded_weight = loaded_weight.narrow(output_dim, start_idx,
                                                     shard_size)
...
@CustomOp.register("intern_row_parallel_linear")
class InternRowParallelLinear(LinearBase):
    ...
    def __init__(
        ...
        *,
        ...
        self.tp_size = (get_tensor_model_parallel_world_size()
                        if not disable_tp else 1)
        self.shard_sizes = shard_sizes
        self.input_size_per_partition = self.shard_sizes[self.tp_rank]
        self.output_size_per_partition = output_size
        self.output_partition_sizes = [output_size]
